initialState([[[1,1],[2,6],[3,4],[3,5],[3,8],[4,1],[4,2],[5,7],[6,2],[7,1],[7,3],[7,5]],
...
try_generate_board(N, MoveList,
		   Player, Opponent,[NextPlayer, NewOpponent]):-
	nth1(N, MoveList, Move),
	alter_board(Move, Player, NewPlayer),
	next_generation([NewPlayer, Opponent], [NextPlayer, NewOpponent]).
...
%%	to get minimum number of opponent's pieces
...
%%	loop through each possible move
%%	to get maximum number of player's pieces
...
loopThrough3(N, MoveList, Player, Opponent, Counter, CompareTo, Best):-
	...
	NewN is N-1,
	(   Diff > CompareTo
	->  loopThrough2(NewN, MoveList, Player, Opponent, N, Diff, Best)
	;   loopThrough2(NewN, MoveList, Player,
			 Opponent, Counter, CompareTo, Best)
	).
...
minimax(PlayerColour, [AliveB, AliveR], NewBoardState, Move):-
	(   PlayerColour == 'b'
	->  findBestMove4(AliveB, AliveR, NewBoardState, Move)
	;   findBestMove4(AliveR, AliveB, NewBoardState, Move)
	).

%%      helper function findBestMove4/4
%%	used the feedback from loopThrough4/7 to get the NewBoardState
findBestMove4(Player, Opponent, NewBoardState, Move):-
	findAllMoves(Player, Opponent, PossMoves),
	length(PossMoves, L),
	loopThrough3(L, PossMoves, Player, Opponent, 0, -1, MoveIndex),
	generate_board(MoveIndex, PossMoves,
